...
class IncomingStreamBridge extends Emitter
{
		...
		/** @type {number} */ maxBufferingTime)
	{
		...
		//Listen for multivideotrack evetns
		this.onmultivideotrack = (/** @type number*/ id)=>{
			if (this.stopped) return;
			//Get encoding in native code
			const mediaFrameBridge = /** @type SharedPointer.Proxy<Native.MediaFrameListenerBridgeShared>} */ SharedPointer(this.bridge.GetMultitrackVideo(id));
			//Add encoding to the track
			this.tracks.get("video").addEncoding(String(id),mediaFrameBridge);
		};
