...
class IncomingStreamBridge extends Emitter
{
	...
	* @returns {Array<IncomingStreamTrackBridge>}	- Array of tracks
	...
	detach()
	{
		...
		this.tracks.get("audio")?.setStream(null);
		this.tracks.get("video")?.setStream(null);
	...
	attachTo(/** @type {import("./Stream") | undefined} */ stream)
	{
		...
		this.tracks.get("audio")?.setStream(stream ? stream.stream : null);
		this.tracks.get("video")?.setStream(stream ? stream.stream : null);
