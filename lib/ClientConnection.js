...
/**
 * @typedef {Object} ClientConnectionStats
 * @property {number} inBytes
 * @property {number} outBytes
 */
...
{
	constructor(/** @type {boolean} */ secure = false)
	{
		//Init emitter
		super()
	...
	async publish(/** @type {(outgoingStream : OutgoingStreamBridge) => void} */ callback)
	{
		...
		this.connection.Publish(streamId, this.key);
		...
		outgoingStream.once("stopped", () => {
			...
			if (!this.stopped)
				...
				new Promise((resolve,reject) => this.connection.DeleteStream(streamId, {resolve,reject}))
					.catch(err => Promise.reject(new CommandError('deleteStream', err)));
			//Remove
