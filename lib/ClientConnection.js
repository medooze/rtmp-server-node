...
class ClientConnection extends Emitter
{
	constructor()
	{
		...
		this.onerror = (/** @type {Number} */errorCode) =>
		{
			if (!this.emitterStopped)
				//Emit event
				this.emit("error", this, errorCode);
	...
	stop()
	{
		...
		// Stop emitter later to ensure events triggered by this
		// function call are processed.
		setTimeout(()=> {
			super.stop();
			this.emitterStopped = true;
		}, 0);
