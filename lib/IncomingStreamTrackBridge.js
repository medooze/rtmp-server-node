 ...
 * @property {number} totalBitrate average bitrate (media + RTP overhead) received during last second in bps
...
function getEncodingStatsFromMediaBridge(/** @type {SharedPointer.Proxy<Native.MediaFrameListenerBridgeShared> */ bridge)
{
	...
	const stats = {
		...
		timestamp: Date.now(),
		simulcastIdx: -1,
		codec: mediaStats.numFrames ? bridge.codec : null,
		iframes : bridge.iframes,
		iframesDelta : bridge.iframesDelta,
		bframes : bridge.bframes,
		bframesDelta : bridge.bframesDelta,
		pframes : bridge.pframes,
		pframesDelta : bridge.pframesDelta,
	};
