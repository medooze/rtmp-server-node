 ...
 * @property {number} [audioCodec] Enumeration identifying the type of audio codec in use (See media-server codec.h for details)
 * @property {number} [videoCodec] Enumeration identifying the type of video codec in use (See media-server codec.h for details)
 * 
 * 
 *
	...
	getStats()
	{
		...
		for (let encoding of this.encodings.values())
		{
			...
			if (!this.stats[id] || (Date.now() - this.stats[id].timestamp)>200 )
			{
				...
				if (bridge.audioCodec != Native.UNKNOWN_CODEC)
				{
					this.stats[id].audioCodec = bridge.audioCodec;
				}

				if (bridge.videoCodec != Native.UNKNOWN_CODEC)
				{
					this.stats[id].videoCodec = bridge.videoCodec;
				}
			}
		}
